...
<div class="w-full h-full bg-slate-700 text-white">
  ...
  {#if $role === 'host'}
    <div class="grid grid-cols-[repeat(auto-fill,minmax(300px,1fr))] gap-3 p-5">
      {#each games as game}
        <button
          class="h-60 flex items-center justify-center relative overflow-hidden rounded shadow-xl"
          on:click={() => {
						open(game.id);
					}}
        >
          ...
          <div
            class="absolute font-bold font-mono bg-black/70 p-1 text-2xl text-shadow pointer-events-none bordered-text"
            data-text={game.name}
          >
            {game.name}
          </div>
